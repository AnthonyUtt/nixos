...
prompt="Workspace> "
header="Move which workspace to current output?"
if [[ -n $SWM_FIFO ]]; then
    str=$(cat $SWM_FIFO)
    ...
    i3-msg move workspace to output $focused_output
    notify-send -u low "moved workspace: $selection to output: $focused_output"
    exit
fi
